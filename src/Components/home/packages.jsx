...
const HeroSection = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-screen sm:px-16 md:flex-row relative md:px-0">
      ...
      <div className="bg-[#359dad] hidden shadow-2xl text-white text-2xl md:w-60 md:px-4 md:h-36 md:flex xl:px-12 lg:px-6 lg:h-32 font-bold xl:w-60 xl:h-40  lg:w-60 md:items-center md:justify-center md:block  transform hover:scale-105 hover:cursor-pointer" onClick={() => setImage(design)}>
        DESIGN
      </div>
      ...
      <div className="relative transform hover:scale-105 h-auto  max-w-full md:max-w-3/4 lg:w-2/3 xl:w-1/2 2xl:w-1/4">
        <img
          src={img}
          alt="Phone1"
          className="w-100 h-auto p-2 my-32 object-cover"
        />
        <div className="absolute inset-0 flex items-center justify-center md:my-0">
          ...
          />
          <Scroller className="block md:hidden" />
        </div>
      </div>
      <div className="bg-[#34dd64] text-white shadow-2xl hidden text-2xl md:w-60 md:h-36 md:px-4 lg:px-4  font-bold lg:h-32 xl:w-60 xl:px-10 xl:h-40 ml-3 mr-20 md:flex md:block md:items-center md:justify-center  transform hover:scale-105 hover:cursor-pointer" onClick={() => setImage(devliver)}>
        DELIVER
      </div>
